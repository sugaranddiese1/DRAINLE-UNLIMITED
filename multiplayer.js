(() => {
  ...
  const els={soloTab:$("#soloTab"),multiplayerTab:$("#multiplayerTab"),soloView:$("#soloView"),multiplayerView:$("#multiplayerView"),brandTagline:$("#brandTagline"),versusTab:$("#versusTab"),coopTab:$("#coopTab"),home:$("#multiplayerHome"),modeLabel:$("#multiplayerModeLabel"),title:$("#multiplayerTitle"),description:$("#multiplayerDescription"),backendNotice:$("#backendNotice"),playerName:$("#playerName"),versusOptions:$("#versusOptions"),create:$("#createRoomButton"),join:$("#joinRoomButton"),joinCode:$("#joinCode"),message:$("#multiplayerMessage"),lobby:$("#roomLobby"),roomCode:$("#roomCode"),connection:$("#connectionStatus"),copyInvite:$("#copyInviteButton"),copyCode:$("#copyCodeButton"),hostName:$("#hostName"),guestName:$("#guestName"),hostStatus:$("#hostStatus"),guestStatus:$("#guestStatus"),ready:$("#readyButton"),start:$("#startMatchButton"),leave:$("#leaveRoomButton"),lobbyMessage:$("#lobbyMessage"),game:$("#multiplayerGame"),roundLabel:$("#roundLabelMultiplayer"),matchScore:$("#matchScore"),opponentName:$("#opponentName"),opponentProgress:$("#opponentProgress"),coopLock:$("#coopLockStatus"),search:$("#multiplayerSongSearch"),suggestions:$("#multiplayerSuggestions"),guess:$("#multiplayerGuessButton"),guessStatus:$("#multiplayerGuessStatus"),gameLeave:$("#multiplayerLeaveButton"),coopDecision:$("#coopDecision"),coopChoices:$("#coopChoices"),coopDecisionStatus:$("#coopDecisionStatus"),gameMessage:$("#multiplayerGameMessage"),boardLayout:$("#versusBoardLayout"),opponentBoard:$("#opponentBoard"),opponentBoardName:$("#opponentBoardName"),opponentFeedbackRows:$("#opponentFeedbackRows"),rows:$("#multiplayerGuessRows"),results:$("#roundResults"),resultTitle:$("#multiplayerResultTitle"),answer:$("#multiplayerAnswer"),comparison:$("#resultComparison"),resultSummary:$("#multiplayerResultSummary"),share:$("#multiplayerShareButton"),next:$("#nextRoundButton"),rematch:$("#rematchButton"),return:$("#returnMultiplayerButton")};
  const normalize=value=>String(value||"").toLowerCase().normalize("NFKD").replace(/[^a-z0-9]+/g," ").trim();
  const MAX_GUESSES=10;
  let mode="versus",matchLength=1,user=null,room=null,channel=null,selected=null,history=[],opponentHistory=[],matchSummary=[],activeSuggestion=-1,autoSubmitting=false;
  ...
  function setMode(next,force=false){if(room&&!force)return;mode=next;const coop=mode==="coop";els.versusTab.classList.toggle("active",!coop);els.coopTab.classList.toggle("active",coop);els.versusTab.setAttribute("aria-selected",String(!coop));els.coopTab.setAttribute("aria-selected",String(coop));els.modeLabel.textContent=coop?"two player team":"1 versus 1";els.title.textContent=coop?"co-op":"versus";els.description.textContent=coop?"Lock answers separately, agree on one guess, and solve together.":"The same mystery song. Separate boards. Fewest guesses wins.";els.create.textContent=coop?"create co-op room":"create game";els.join.textContent=coop?"join co-op room":"join game";els.versusOptions.hidden=coop;els.message.textContent=""}
  ...
  async function enterRoom(code){await loadRoom(code);mode=room.mode;setMode(mode,true);history=[];opponentHistory=[];matchSummary=[];setScreen(room.status==="lobby"?"lobby":room.status==="playing"?"game":"results");subscribeRoom();history=await loadHistory();opponentHistory=await loadOpponentHistory();matchSummary=await loadMatchSummary();renderRoom();const url=new URL(location.href);url.searchParams.set("multiplayer",mode);url.searchParams.set("room",room.code);window.history.replaceState({},"",url)}
  ...
  async function loadOpponentHistory(){if(!room||room.mode!=="versus"||room.status==="lobby")return[];try{return await rpc("get_blaidle_opponent_progress",{p_code:room.code})||[]}catch{return[]}}
  ...
  function renderLobby(){els.roomCode.textContent=room.code;els.hostName.textContent=room.host_name||"waiting...";els.guestName.textContent=room.guest_name||"waiting for player";els.hostStatus.textContent=room.host_ready?"ready":"not ready";els.guestStatus.textContent=room.guest_ready?"ready":room.guest_id?"not ready":"not connected";els.hostStatus.classList.toggle("ready",room.host_ready);els.guestStatus.classList.toggle("ready",room.guest_ready);const ready=isHost()?room.host_ready:room.guest_ready;els.ready.textContent=ready?"not ready":"ready";els.start.hidden=!isHost();els.start.disabled=!(room.host_ready&&room.guest_ready);els.lobbyMessage.textContent=room.host_ready&&room.guest_ready?(isHost()?"Both players are ready. Start when you are ready.":"Both players are ready. Waiting for the host."):"The game starts after both players are ready."}
  function renderGame(){els.roundLabel.textContent=room.mode==="versus"?`round ${room.current_round} of ${room.match_length}`:"co-op round";els.matchScore.textContent=room.mode==="versus"?`${room.host_name} ${room.host_score} — ${room.guest_score} ${room.guest_name}`:`${room.host_name} + ${room.guest_name}`;const other=otherSide();const otherCount=room[`${other}_guess_count`]||0;const otherOutcome=room[`${other}_outcome`]||"playing";els.opponentName.textContent=room.mode==="versus"?(otherName()||"opponent"):"team progress";els.opponentProgress.textContent=room.mode==="versus"?`guess ${otherCount} · ${otherOutcome}`:`${room.coop_guess_count||0}/${MAX_GUESSES} guesses`;els.rows.innerHTML="";history.forEach((item,index)=>els.rows.appendChild(renderHistoryRow(item,index)));renderOpponentBoard();const count=room.mode==="coop"?(room.coop_guess_count||0):(room[`${ownSide()}_guess_count`]||0);els.guessStatus.textContent=`${Math.max(0,MAX_GUESSES-count)} guess${MAX_GUESSES-count===1?"":"es"} remaining`;const ownFinished=room.mode==="versus"&&room[`${ownSide()}_outcome`]!=="playing";const ownLocked=room.mode==="coop"&&room[`${ownSide()}_locked`];const deciding=room.mode==="coop"&&room.coop_proposal_host!==null&&room.coop_proposal_guest!==null;els.search.disabled=ownFinished||ownLocked||deciding;els.guess.disabled=ownFinished||ownLocked||deciding||!selected;els.coopLock.hidden=room.mode!=="coop"||!ownLocked||deciding;els.coopLock.textContent="your guess locked ✓ · waiting for teammate...";els.coopDecision.hidden=!deciding;els.gameMessage.textContent=ownFinished?`You are ${room[`${ownSide()}_outcome`]}. Waiting for your opponent...`:"";if(deciding)renderCoopDecision();else{els.coopChoices.innerHTML="";autoSubmitting=false}}
  function renderOpponentBoard(){
    const versus=room.mode==="versus";
    els.opponentBoard.hidden=!versus;
    els.boardLayout.classList.toggle("has-opponent",versus);
    if(!versus)return;
    ...
    opponentHistory.forEach((states,index)=>{
      ...
      row.setAttribute("aria-label",`opponent guess ${index+1}: ${states.join(", ")}`);
  ...
  els.soloTab.addEventListener("click",()=>showView("solo"));els.multiplayerTab.addEventListener("click",()=>showView("multiplayer"));els.versusTab.addEventListener("click",()=>setMode("versus"));els.coopTab.addEventListener("click",()=>setMode("coop"));document.querySelectorAll("[data-rounds]").forEach(button=>button.addEventListener("click",()=>{matchLength=Number(button.dataset.rounds);document.querySelectorAll("[data-rounds]").forEach(item=>item.classList.toggle("active",item===button))}));els.create.addEventListener("click",createRoom);els.join.addEventListener("click",joinRoom);els.joinCode.addEventListener("input",()=>els.joinCode.value=els.joinCode.value.toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,5));els.copyInvite.addEventListener("click",()=>copyText(roomUrl(),els.copyInvite));els.copyCode.addEventListener("click",()=>copyText(room.code,els.copyCode));els.ready.addEventListener("click",toggleReady);els.start.addEventListener("click",startMatch);els.leave.addEventListener("click",leaveRoom);els.gameLeave.addEventListener("click",leaveRoom);els.next.addEventListener("click",nextRound);els.rematch.addEventListener("click",rematch);els.return.addEventListener("click",leaveRoom);els.share.addEventListener("click",shareResults);els.search.addEventListener("input",showSuggestions);els.search.addEventListener("keydown",event=>{const items=[...els.suggestions.querySelectorAll(".suggestion")];if(event.key==="ArrowDown"||event.key==="ArrowUp"){event.preventDefault();if(items.length){activeSuggestion=(activeSuggestion+(event.key==="ArrowDown"?1:-1)+items.length)%items.length;items.forEach((item,index)=>item.classList.toggle("active",index===activeSuggestion));items[activeSuggestion].scrollIntoView({block:"nearest"})}}else if(event.key==="Enter"){event.preventDefault();if(!els.suggestions.hidden&&activeSuggestion>=0)items[activeSuggestion].click();else if(selected)submitGuess()}else if(event.key==="Escape")els.suggestions.hidden=true});els.guess.addEventListener("click",submitGuess);document.addEventListener("click",event=>{if(!event.target.closest("#multiplayerSearchWrap"))els.suggestions.hidden=true});

  els.playerName.value=localStorage.getItem("blaidle-player-name")||"";if(!configured){els.backendNotice.hidden=false;els.backendNotice.textContent="Multiplayer is installed but needs Supabase configuration before rooms can be created.";els.create.disabled=true;els.join.disabled=true}const params=new URLSearchParams(location.search);if(params.get("multiplayer")){showView("multiplayer");setMode(params.get("multiplayer")==="coop"?"coop":"versus");if(params.get("room")){els.joinCode.value=params.get("room").toUpperCase();setMessage("Enter your name, then join the invited room.")}}else setMode("versus");
})();
